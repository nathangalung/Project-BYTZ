...
const EXPECTED_LIST_COLUMNS = [
  ...
  'finalPrice',
  'id',
  // A dispute and a hold left project_status when it collapsed to nine
  // positions. They are still what a list reader decides on, so they ship as
  // their own facts: a derived flag and the column that replaced the status.
  'isDisputed',
  'onHoldAt',
  'ownerId',
  'platformFee',
...
runIf('ProjectRepository', () => {
  ...
  /**
   * An invitation sent to one talent. team_forming used to say a project was
   * waiting on replies; the status collapsed into matching, so the wait is now
   * only visible as a live assignment the talent has not answered.
   */
  ...
  describe('updateStatus', () => {
    ...
    it('records the transition it came from', async () => {
      const id = await seedProject({ status: 'brd_review' })

      await repo.updateStatus(id, 'prd_review', ownerId, 'PRD generated by AI')
    ...
    it('publishes the dedicated completion subject naming the owner', async () => {
      const id = await seedProject({ status: 'final_review' })
    ...
    it('publishes no completion subject for any other transition', async () => {
      ...
      await repo.updateStatus(id, 'final_review', ownerId)
  ...
  describe('findStalledTeamFormation', () => {
    it('returns team projects still awaiting a reply, oldest first', async () => {
    ...
    it('ignores a single-talent project awaiting a reply', async () => {
      const id = await seedProject({ status: 'matching', teamSize: 1 })
      await invite(id)
    ...
    /**
     * matching now spans both halves of what used to be matching and
     * team_forming, so an unanswered invitation is the only thing separating a
     * team that is stuck from one that is simply still being sourced.
     */
    it('ignores a team project nobody has been invited to yet', async () => {
    ...
    it('ignores projects in any other status', async () => {
      const id = await seedProject({ status: 'in_progress', teamSize: 3 })
      await invite(id)
    ...
    it('hides a soft-deleted project', async () => {
      const id = await seedProject({ status: 'matching', teamSize: 3, deletedAt: new Date() })
      await invite(id)
    ...
    it('honours the batch limit', async () => {
      const first = await seedProject({ status: 'matching', teamSize: 2 })
      await invite(first)
      const second = await seedProject({ status: 'matching', teamSize: 2 })
      await invite(second)
  ...
  describe('getStatusLogs', () => {
    it('returns the audit trail newest first', async () => {
      const id = await seedProject({ status: 'draft' })
      await repo.updateStatus(id, 'scoping', ownerId)
      await repo.updateStatus(id, 'brd_review', ownerId)

      const logs = await repo.getStatusLogs(id)

      expect(logs.map((l) => l.toStatus)).toEqual(['brd_review', 'scoping'])
    })
