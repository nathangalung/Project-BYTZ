...
func TestLoad_FullConfig(t *testing.T) {
	clearEnv(t)
	t.Setenv("DATABASE_URL", "postgres://prod:secret@db.example.com:5432/kerjacus")
	t.Setenv("MIDTRANS_SERVER_KEY", "SB-Mid-server-test")
	t.Setenv("PORT", "8080")
	t.Setenv("CORS_ORIGIN", "https://app.kerjacus.id")
	...
	if cfg.DatabaseURL != "postgres://prod:secret@db.example.com:5432/kerjacus" {
		t.Errorf("DatabaseURL mismatch")
	}
	if cfg.Port != "8080" {
		t.Errorf("Port = %q, want %q", cfg.Port, "8080")
	}
	if cfg.CORSOrigin != "https://app.kerjacus.id" {
		t.Errorf("CORSOrigin = %q, want %q", cfg.CORSOrigin, "https://app.kerjacus.id")
	}
