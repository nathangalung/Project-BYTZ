...
func TestReleaseEscrow_Success(t *testing.T) {
	...
	txnMock := &store.MockTransactionStore{
		...
		// derive it from. 1 juta pays the talent 92%, making the fee on a 50,000
		// milestone 4,000.
		GetMilestonePricingFn: func(_ context.Context, _, _ string) (*store.MilestonePricing, error) {
			price, payout := int64(1_000_000), int64(920_000)
	...
	body := `{"milestoneId":"ms-1","projectId":"proj-1","talentId":"talent-1","amount":50000,"feeAmount":4000,"performedBy":"system","idempotencyKey":"rel-k-1"}`
	req := httptest.NewRequest("POST", "/api/v1/payments/internal/release", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
...
func TestReleaseEscrow_ServiceError(t *testing.T) {
	...
	body := `{"milestoneId":"ms-1","projectId":"proj-1","talentId":"talent-1","amount":50000,"feeAmount":4000,"performedBy":"system","idempotencyKey":"rel-k-3"}`
	req := httptest.NewRequest("POST", "/api/v1/payments/internal/release", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
...
func TestCreateSnapToken_SuccessHandler(t *testing.T) {
	...
	body := `{"projectId":"p-1","checkoutType":"brd","itemName":"BRD","customerName":"User","customerEmail":"u@e.com"}`
...
func TestCreateSnapToken_DeniedForNonOwner(t *testing.T) {
	...
	body := `{"projectId":"p-1","checkoutType":"brd","customerEmail":"u@e.com"}`
	req := httptest.NewRequest("POST", "/api/v1/payments/create-snap-token", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-User-ID", "stranger")
