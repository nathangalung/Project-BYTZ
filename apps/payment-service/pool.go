...
// line. Pinning it makes the arithmetic fixed: 30 (TS) + 18 (Go: 6 here,
// 8 notification, 4 admin) = 48 client connections, 24% of MAX_CLIENT_CONN,
...
//
// maxConns 6 here: HTTP payment requests and Midtrans settlement callbacks,
// plus the disbursement reconciliation sweep - which is one ticker goroutine
// running one pass at a time (internal/service/reconcile.go:112-126), so it
// adds one connection to the ceiling, not a fan-out.
const (
	maxConns = 6
