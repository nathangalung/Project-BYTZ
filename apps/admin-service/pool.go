...
// = 30, plus the three Go services. ai-service is not in the budget; it
...
// 8 notification, 4 here) = 48 client connections, 24% of MAX_CLIENT_CONN,
// with a worst-case simultaneous demand of 48 against 20 server slots.
...
//
// maxConns 4 here, the smallest of the three: admin is a handful of internal
// operators, not public traffic, and its expensive read - the dashboard
// aggregate - is answered from a 30s in-process cache
// (internal/handler/dashboard.go). Four also means a runaway dashboard range
// cannot pin more than a fifth of pgbouncer's server slots.
const (
	maxConns = 4
	// One warm, so the first request after an idle period does not pay a TCP
	// handshake plus pgbouncer auth. Lower than the other two because admin
	// traffic is bursty and mostly absent.
	minConns = 1
