...
import { useWorkerProfile as useWorkerProfileHook } from '@/hooks/use-workers'
...
function useWorkerRatings() {
  return useQuery({
    queryKey: ['worker-ratings'],
    queryFn: async () => {
      const res = await fetch(`${API_URL}/api/v1/workers/ratings`, {
        credentials: 'include',
      })
...
function ProfileContent({ t }: { t: (key: string, fallback: string) => string }) {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useWorkerProfileHook(user?.id ?? '') as {
    data: WorkerProfile | undefined
    isLoading: boolean
    isError: boolean
  }
    ...
    return (
      <EmptyState
        ...
        description={t(
          ...
          'Lengkapi profil worker Anda untuk mulai menerima proyek',
        )}
      />
    )
  }
...
function RatingHistorySection({ t }: { t: (key: string, fallback: string) => string }) {
  const { data: ratings, isLoading } = useWorkerRatings()
