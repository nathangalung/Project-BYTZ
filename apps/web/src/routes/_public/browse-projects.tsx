...
async function fetchPublicProjects(category?: string, page = 1) {
  ...
  try {
    const res = await fetch(apiUrl(`/api/v1/projects/public?${params}`))
  ...
  return (
    <div>
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        {/* Category filter */}
        <nav aria-label={t('filter_by_category')} className="mt-6 flex flex-wrap gap-2">
          {CATEGORIES.map((c) => (
            <button
              type="button"
              key={c}
        ...
        {/* Grid */}
        <h2 className="sr-only">{t('project_list')}</h2>
        {loading ? (
