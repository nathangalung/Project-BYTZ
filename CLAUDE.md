...
- Sebelas file route punya plafon yang tidak bisa dilewati: plugin router menempelkan blok `import.meta.hot` yang selalu undefined di bawah `vitest run`, jadi tiap file route membawa satu function dan dua branch yang permanen tidak tercakup. 100% per file route mustahil, dan threshold 100% akan gagal di hari pertama. Menghapusnya adalah opsi plugin router atau exclusion coverage, bukan sesuatu yang bisa dicapai test
