...
- API Testing: tidak ada. Bruno pernah disebut di sini beserta "collections disimpan di repo", padahal tidak ada satu pun file `.bru`. Yang benar-benar melatih endpoint adalah 23 suite integrasi di `apps/project-service/src/routes/` yang mengirim HTTP request ke Postgres sungguhan
...
Tidak ada `packages/ui` berisi komponen, dan itu keputusan yang sudah diukur, bukan kelalaian: `apps/web/src/components/ui` punya 8 komponen, `apps/admin/src/components/ui` punya 7, dan hanya SATU nama yang beririsan, `error-boundary.tsx`. Keduanya implementasi terpisah dan sengaja begitu, tapi ia satu-satunya kandidat nyata untuk `packages/ui-kit` kalau nanti ada yang menyatukannya. Marketplace publik dan admin console memang butuh vocabulary berbeda. Yang benar-benar terduplikasi cuma formatter dan token, dan itu yang masuk `packages/ui-kit`.

Web dulu punya 12. Badge, Button, Card, dan Input dihapus karena tidak ada satu pun importer di luar test-nya sendiri: route menulis markup-nya sendiri, jadi keempatnya 553 baris dan 31 test yang menguji sesuatu yang tidak dipakai siapa pun. Modal ada di daftar yang sama tapi TIDAK dihapus, dan alasannya cacat bukan selera. Tiga route menulis dialog `fixed inset-0 z-50` sendiri; dua tanpa focus management sama sekali, dan satu menyatakan `aria-modal="true"` — yang memberi tahu assistive technology bahwa sisa halaman inert — sambil membiarkan Tab keluar begitu saja. Modal mengerjakan yang dijanjikan atribut itu (Escape, Tab trap, focus awal, focus dikembalikan), jadi ketiganya sekarang memakainya. DUA dialog tangan lain belum ikut pindah dan keduanya tanpa focus management: `milestones.tsx` dan `milestone-detail.tsx`. Sebelum menambah komponen baru ke direktori itu, tambahkan bersama call site pertamanya.

Format Rupiah ringkas melipat ke juta sampai atas, jadi satu miliar tampil `Rp 1.000 jt` dan 2,5 miliar tampil `Rp 2.500 jt`. Admin panel dulu memakai `Rp 2.5M` dan web tidak pernah memakai M sama sekali. Menyeragamkan ke M berarti memperkenalkannya di halaman yang menampilkan pengeluaran kumulatif owner dan penghasilan kumulatif talenta, dan M yang terbaca sebagai juta alih-alih miliar adalah salah baca seribu kali lipat atas uang orang. `jt` selalu berarti juta. Kolom yang lebih lebar adalah harga dari angka yang tidak bisa disalahbaca.
...
- Konvensi ini berlaku sejak migrasi 0024 dan TIDAK retroaktif: 22 file yang membuat index atau menjalankan ALTER TABLE tidak punya `SET lock_timeout` maupun `SET statement_timeout`, terbanyak 0000 (78 pernyataan DDL) plus 0001-0013, 0015-0017 dan 0019-0022. Satu yang ditulis SETELAH konvensi dimulai dan tetap terlewat adalah 0033, sebuah `ALTER TABLE ... DROP COLUMN` telanjang. Migrasi baru wajib memakainya. CONCURRENTLY tidak tersedia karena drizzle membungkus tiap file migrasi dalam satu transaksi (pg-core/dialect.cjs) dan Postgres menolak CONCURRENTLY di dalam transaksi. Timeout tidak menghapus lock, hanya membatasinya, supaya migrasi yang akan mengantre di belakang write gagal cepat dan bisa diulang di window yang tenang
...
Daftar pengecualian route-to-db memuat DELAPAN BELAS file yang disebut satu
per satu, bukan warning selimut. Komentar di dalam config-nya masih menulis
"seventeen" dan sudah salah terhadap daftarnya sendiri; daftar itu tumbuh satu. Warning yang menyala sejak hari ia ditulis adalah
...
- Input sanitization di semua user-facing endpoint lewat validasi Zod. Tidak ada sanitizer HTML: DOMPurify tidak terpasang
...
- Coverage target: 100% adalah TUJUAN, bukan angka yang terkonfigurasi, dan jarak keduanya harus disebut supaya tidak disangka sudah tercapai. Threshold yang benar-benar berlaku: apps/web 96/91/94/97 dan apps/admin 97/94/96/98 (statement/branch/function/lines) di `vite.config.ts` masing-masing; `vitest.shared.ts` sendiri tidak menyetel threshold apa pun, `coverageConfig()` menerimanya sebagai parameter. Menaikkannya ke 100 akan gagal di hari pertama karena plugin router menyuntikkan blok `import.meta.hot` tanpa syarat ke setiap file route, dan itu hanya bisa dihapus lewat opsi plugin atau exclusion coverage, bukan lewat test tambahan. Sebelumnya 80% untuk business logic; dinaikkan atas keputusan owner. Yang TIDAK berubah: baris yang dieksekusi tanpa assertion lebih buruk daripada baris yang tidak tercakup, karena ia menaikkan angka sambil menghapus sinyal. Setiap test harus bisa gagal karena alasan nyata, dan cara membuktikannya adalah merusak implementasinya lalu melihat test itu merah
...
- Setiap file route punya plafon yang tidak bisa dilewati: plugin router menempelkan blok `import.meta.hot` TANPA syarat, jadi ia mendarat di web maupun admin. File-nya `core/hmr/vite-adapter.js` yang dipilih lewat `core/hmr/select-adapter.js` dan disuntikkan dari `router-hmr-plugin.js`; tidak ada file bernama `route-hmr-statement.js`. Biayanya LEBIH dari satu statement, satu function, dan dua branch: blok itu memuat `??=`, dua ternary, dua rantai `?.`, satu if tiga cabang, dan callback accept dengan if serta `??` miliknya sendiri. 35 file route di web dan 11 di admin. Cara membedakannya dari branch asli: item HMR selalu mulai di kolom <= 2 karena ia statement top-level yang ditempelkan. 100% per file route mustahil dan threshold 100% akan gagal di hari pertama; menghapusnya adalah opsi plugin atau exclusion coverage, bukan sesuatu yang bisa dicapai test
...
- XSS: React auto-escaping saja. DOMPurify TIDAK terpasang di package.json mana pun, dan bagian ini dulu menyebutnya sebagai mitigasi yang berjalan. Yang menahan XSS hari ini cuma escaping bawaan React plus nol pemakaian `dangerouslySetInnerHTML` di apps/web maupun apps/admin. Pasang DOMPurify bersama call site pertamanya, jangan sebagai klaim
